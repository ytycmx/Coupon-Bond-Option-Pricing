...
for i = J+1
   ...
   for j = 2:2*i-2
       V(i,j) = exp(-dt*r(i,j))*(p(i,3*j-2)*(V(i+1,j-1)+F*c1)+p(i,3*j-1)*(V(i+1,j)+F*c1)+p(i,3*j)*(V(i+1,j+1)+F*c1));
   end
   for j = 2*i-1
       V(i,j) = exp(-dt*r(i,j))*(p(i,3*j-2)*(V(i+1,j-2)+F*c1)+p(i,3*j-1)*(V(i+1,j-1)+F*c1)+p(i,3*j)*(V(i+1,j)+F*c1));
   end
end
for i = J:-1:3
    if i==6 || i==4
...
for i = J+1
   for j = 1
      V1(i,j) = exp(-dt*r(i,j))*(p(i,3*j-2)*(V1(i+1,j)+F*c1)+p(i,3*j-1)*(V1(i+1,j+1)+F*c1)+p(i,3*j)*(V1(i+1,j+2)+F*c1));
   end
   for j = 2:2*i-2
       V1(i,j) = exp(-dt*r(i,j))*(p(i,3*j-2)*(V1(i+1,j-1)+F*c1)+p(i,3*j-1)*(V1(i+1,j)+F*c1)+p(i,3*j)*(V1(i+1,j+1)+F*c1));
   end
   for j = 2*i-1
       V1(i,j) = exp(-dt*r(i,j))*(p(i,3*j-2)*(V1(i+1,j-2)+F*c1)+p(i,3*j-1)*(V1(i+1,j-1)+F*c1)+p(i,3*j)*(V1(i+1,j)+F*c1));
   end
end
for i = J:-1:3
    if i==6 || i==4
